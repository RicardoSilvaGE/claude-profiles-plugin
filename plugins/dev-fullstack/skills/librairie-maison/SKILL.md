...
description: "Socles de démarrage vérifiés pour ne pas repartir de zéro : jeu VANILLA (PWA hors-ligne, React in-browser sans build, server.js Node, HTTPS local auto-signé, génération docx/PDF) et jeu NEXT-TS (le défaut déclaré du profil). À charger avant d'écrire le squelette d'une nouvelle application, ou d'ajouter à une application existante un service worker, un serveur local, un export PDF ou un lancement Windows silencieux."
---
...
| **NEXT-TS** | `next-ts/` | C'est le **défaut déclaré du profil** (Next.js App Router + TypeScript strict + Tailwind + shadcn). Un seul dépôt s'en approche aujourd'hui. |
